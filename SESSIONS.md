...
## 2026-08-01 (20) — Vendetta al mercado real: las premium por fin cruzadas + foils nuevos
- **💰 157 precios de Vendetta al mercado real de TCGplayer** (día después del release). Lo nuevo: el cruce v3 por **nombre exacto con variante** logró tasar las **50 premium (Overnumbered/Alternate Art)** que todas las revisiones anteriores dejaban "sin cruce" — el inventario sí guarda la variante en el nombre, solo que con `,` donde TCGplayer usa `-` y con prefijos tipo `Yordle,`. Regla de seguridad: una variante solo cruza con la MISMA variante; sin sufijo solo cruza con premium si la carta NO existe en versión base (Jayce Man of Progress, Kha'Zix, etc. solo existen Overnumbered).
  - **El colapso post-preventa golpeó durísimo a las premium:** Swain Visionary ON ₡1.040.000 → ₡55.000; Irelia ON ₡365.000 → ₡92.000; Diana ON ₡290.000 → ₡57.000; los Alternate Art cayeron aún más en % (Mel Defiant Soul AA ₡21.000 → ₡1.000 — Andrés la vio "super alta" en la página y era esto). Subieron 10, destacando Renekton Brute ON ₡100 (placeholder) → ₡40.000 y Akali Rogue Assassin ON ₡91.000 → ₡155.000.
  - Verificación en vivo carta por carta (mpapi) de las 50 premium: 32 exactas, 18 con deriva chica del snapshot → afinadas al precio vivo. Guardián anti-centinela descartó 9 precios basura. Foils y stock verificados intactos contra backup.
- **✨ 76 precios foil NUEVOS agregados** (75 Vendetta + Order Rune de Origins): TCGplayer ya publicó los foils de las comunes/incomunes de Vendetta. Método del proyecto (Foil USD × 520 + redondeo) + **`stockf: 0` explícito** (cero foils fantasma). Criterio de selección seguro: solo cartas con AMBAS impresiones en TCGplayer (las foil-only quedan fuera solas). Rebuild de fichas (`make_cartas.py`, 0 slugs cambiados) porque el toggle Normal/Foil se hornea en el HTML. Verificado en producción: toggle y foil ₡1.200 en Disciple of Shen.
  - Curiosidad de mercado: Threshold of the Gray tiene el foil MÁS BARATO que la normal ($0.28 vs $0.59) — real, se aplicó tal cual.
  - ⚠️ **Pendiente detectado:** la normal de **Order Rune (Origins) está a ₡3.300 y el mercado la tiene en $0.12 (₡100)** — muy desactualizada, decidir si bajarla.
- Commits: `fb4e836f` (157 precios) · `6a011d1c` (18 afinadas al vivo) · `9fd69eb6` (76 foils + fichas). Todo en producción.
...
- **🔧 El ✓ del panel no completaba — arreglado de raíz.** Andrés confirmaba y no pasaba nada. Se descartó lo descartable: token y claves OK, endpoints vivos, y **no era un 409 de stock** (se corrió la lógica real de `_lib.js` en Node contra los archivos: daba `200 OK`). La causa mas probable es el TIEMPO: confirmar lee `productos.json` (1.4 MB) y lo reescribe con un PUT de 1.9 MB en base64 → **5-11 s contra el límite de 10 s** del plan.
  - 🚫 **Intento fallido que hay que recordar:** agregar `functions.maxDuration` a `vercel.json`. **Vercel lo rechaza en este plan y el BUILD FALLA**, así que dejó de salir cualquier deploy — el sitio seguía sirviendo la versión vieja y parecía que los push "no hacían nada". Revertido. Documentado en CLAUDE.md.
  - **Fix real, por dos lados:** (1) `readJsonFile` hacía dos peticiones a GitHub en serie siendo independientes → ahora `Promise.all`, ~1 s menos; (2) nuevo **`L.stockYaDescontado(pedId)`** que mira el historial de commits de `productos.json` buscando `Pedido <id> confirmado:` — si un intento se cortó DESPUÉS de escribir el stock pero ANTES de marcar el pedido, al reintentar **ya no se descuenta doble** (antes sí, y en silencio); (3) el panel ante un corte re-consulta el pedido hasta 4 veces y avisa el estado REAL.
  - **R-0015 (₡17.800) y R-0021 (₡12.700) se confirmaron por script**, replicando `pedido-accion.js`. R-0021 chocaba en Disarming Rake foil: había una sola y se la llevó Jean Pierre (R-0025); Andrés confirmó que tenía otra y se repuso. ⚠️ **Esas dos ventas NO quedaron en la sección Ventas** (eso lo escribe el navegador, no el repo): ₡30.500 a anotar a mano.
  - 🤖 **Lección de proceso:** mis ~30 `curl` seguidos activaron el **Vercel Security Checkpoint** (403 a mi IP, no a los clientes). Para verificar producción, usar el navegador, no ráfagas de curl.

- **🐞 No se podía rechazar un pedido expirado.** Andrés pidió varias veces limpiar el de Kevin Fonseca (R-0024) y no podía: el botón «Rechazar» **solo se renderizaba en pedidos PENDIENTES**, y ese ya estaba EXPIRADO, así que el panel solo le ofrecía «Confirmar venta». No era terquedad suya — la acción no existía en pantalla. Arreglado: el botón ✕ Rechazar sale también en expirados (la API siempre lo aceptó, solo bloquea rechazar confirmados). Bandeja en **0 pendientes**.
...
- **🖼️ Las imágenes de Vendetta estaban CORRIDAS (96 cartas con el arte equivocado).** Andrés vio que «Disciple of Shen 117/166» mostraba a Diana. No era error de datos: **el CDN de TCGplayer sirve la carta ANTERIOR** en el bloque de productIds `707605-707700`. La imagen de `707627` mostraba a Jayce 194/166 aunque `707627` ES Diana, y la de `707628` mostraba a Diana aunque ES Disciple of Shen. El bloque `705996-706126` estaba bien (verificado con Dominus 142/166).
  - **Solución: pasar Vendetta a las imágenes OFICIALES de Riot**, igual que el resto de Riftbound, en vez de parchar el corrimiento (TCGplayer lo va a arreglar y rompería de nuevo). Se sacan del payload de Next.js de la galería oficial (playriftbound.com), que trae cada carta con su `id` (`ven-117-166`) y su `cardImage`. **227 imágenes** cambiadas en base + catálogo del panel.
  - ⚠️ **`catalogo/riftbound_rich.json` hubo que RE-KEYEARLO**: las fichas se cruzan por URL de imagen, así que sin eso cambiaban los 227 slugs. Verificado **0 slugs rotos**. Validación automática de las 227: el nombre de cada carta coincide con el que Riot le da a ese número (**0 desajustes, 0 imágenes duplicadas**).
- **👁️ La casilla «Mostrar agotadas» ahora MANDA siempre.** Andrés reportó que salían agotadas con la casilla sin marcar: era la regla del 24/07 (filtrar/buscar las mostraba igual), pero en la práctica la casilla decía "no" y salían de todos modos → se veía roto. Sin marcar, la grilla enseña **solo lo comprable**, también al filtrar y al buscar. **Lo que NO cambia** (y es lo que evita que un set sin stock desaparezca): los conteos de los dropdowns siguen incluyendo agotadas y el buscador del hero las sigue encontrando con su tag «Agotado». Además el mensaje de vacío mentía ("no encontramos eso" cuando SÍ las tenemos, solo agotadas) → ahora dice «Ahora mismo no tenemos disponible nada de esto» + botón **«Ver N cartas agotadas»** que marca la casilla.
- **🔢 La tienda ordena la grilla por número de carta** (antes "Relevancia", que no ordenaba nada). Mismo orden natural que el panel: el arte alternativo pegado a su base (`019/166`, `019a/166`, `020/166`), runas y Signatures al final, sellado arriba. **No reacomoda juegos ni expansiones entre sí** — respeta el orden del inventario y ordena solo DENTRO de cada expansión.
  - **One Piece también**: SÍ tenía el número, pero `rich_onepiece` lo guarda en `number` (`ST30-005`) y `rich_riftbound` en `collector_number`+`code`; `cartas.json` solo leía el de Riftbound. El índice ahora cae al otro campo → **2.577 de 2.629** ordenables. ⚠️ El `num` que arma el SLUG NO se tocó: cambiarlo habría renombrado las 2.629 fichas de One Piece. Verificado 0 slugs cambiados, 0 fichas HTML modificadas.
- **Conflicto de rebase con el publish del panel:** Andrés publicó su stock mientras yo tenía el cambio de imágenes; los dos tocaban `productos.json`. Se resolvió tomando SU versión completa (la del stock) y re-aplicando las imágenes con Python — **verificado que las 1.088 unidades quedaron idénticas**. Nunca resolver ese archivo a mano.
- Cache-busting `?v=75` → **`?v=77`** (index y juego).

## 2026-07-27 (18b) — Catálogo del panel por número + revisión de precios de Vendetta
- **Panel: el catálogo ("Agregar desde el catálogo") ahora sale ordenado por número de carta**, como la galería oficial de Riftbound. Antes salía en el orden crudo de TCGplayer (alfabético) y alistar cartas era buscar a ciegas. Orden natural por trozos texto/número: `019/166` < `019a/166` (el arte alternativo pegado a su base), `001/166` antes que `SP1/006`, sellado arriba, y las expansiones NO se reordenan entre sí. **De paso arregló One Piece**: el formato `ST30-005` se comparaba solo por el `30` e ignoraba el `-005` — funcionaba nada más porque el archivo ya venía ordenado. Verificado en los 5 catálogos (0 entradas perdidas) y en el panel real que el `+` sigue agregando la carta de su fila (el reorden cambia los índices).
- **🚨 Trampa grande: el `marketPrice` de TCGplayer MIENTE en sets de presale.** `Mel, Defiant Soul (Overnumbered)` reportaba `marketPrice = $0.25` con **todos** los listados en ~$300 → aplicarlo dejaba una carta de ₡160.000 en **₡200**. Aparecieron **12 así en Vendetta**, cuatro con el valor exacto `$0.25` (centinela, no una venta). Ahora el market se descarta si es **< 40% de la mediana de listados**. Documentado en `EXPANSIONES.md` §5 con el código. La rutina semanal no corre el riesgo mientras se use `--aplicar-subidas` (un market basura siempre es más bajo → se ignora).
- **Revisión de precios de Vendetta:** 139 actualizados (16 suben, 123 bajan — el hype de preventa se enfría al entrar más vendedores; Andrés OK con las bajas porque no tiene stock del set). Cobertura **222 → 227/228**. Las 5 que estaban en ₡100 resultaron caras: **Viktor Innovator** e **Irelia Fervent ₡365.000**, Diana No Longer Human ₡290.000, Jayce Defender of Tomorrow ₡210.000, Zed From the Shadows ₡135.000.
- **Verificación por rareza tras aplicar** (para cazar cartas valiosas tiradas en precio ridículo): saltaron 2 → `Shen - Eye of Twilight` (Rare ₡200) es falsa alarma, comprobada contra TCGplayer (listados entre $0.10 y $2.00); `Renekton, Brute (Overnumbered)` es la única sin listados.
- **Foils:** TCGplayer todavía NO abre listados foil aparte de las Common/Uncommon de Vendetta; las 126 Rare/Epic ya SON foil (su `price` es el precio foil). Nada que agregar aún.
- **Pendientes que Andrés dejó para después:** precio a mano de Renekton Brute + los foils cuando TCGplayer los abra (anotados en HANDOFF.md).
...
- **Pre-orden (app.js):** para todo sellado, botón **📦 Pre-ordenar** (verde) + nota del 50% en card y quick-view. **Ajuste posterior (pedido de Andrés): la pre-orden va al CARRITO igual que "Añadir"** (no abre WhatsApp directo) → pasa por el checkout, crea pedido en el panel + WhatsApp, y Andrés confirma/coordina. Se distingue con flag `preorden`:
  - `addToCart(id,foil,preorden)`: línea aparte (`key` `id_pre`), **sin tope de stock** (el sellado puede no haber llegado); pill **"📦 Pre-orden"** + "aparta 50%" en el carrito; `changeQty`/`normalizeCart` respetan el flag.
  - Checkout: el ítem sale marcado `[PRE-ORDEN · aparta 50%]` + nota final "Incluye PRE-ORDEN…"; el payload manda `preorden` por línea.
  - **API:** `api/pedido.js` acepta `preorden` y **no valida stock ni reserva** esas líneas; `_lib.js reservasDe` las salta; `pedido-accion.js` (confirmar) las salta en validación y descuento. `admin.html`: la línea del pedido muestra **📦 PRE-ORDEN · 50%** y el espejo local no descuenta stock de pre-órdenes (registra la venta con `[PRE-ORDEN]`).
- Cache-busting `?v=66→68`. **Verificado en preview:** imagen sellada completa (carga 400px); pre-ordenar → carrito con pill, drawer abierto; `+` sube a qty 3 sin tope (stock 1); mensaje de WhatsApp con línea marcada + nota del 50%; regresión OK (carta normal sin flag ni botón); 4 archivos con sintaxis válida; consola limpia.
