...
- `make_catalogo.py` — **catálogo maestro del panel** (`catalogo/<juego>.json` liviano + `<juego>_rich.json` con efecto/atributos, keyed por img). **5 juegos:** Riftbound y One Piece COMPLETOS; **Pokémon/Magic/Yu-Gi-Oh solo los 12 sets más nuevos** (por `groupId` desc + piso `min_pub`; sus catálogos completos serían gigantes). Riftbound usa **imágenes de Riot** (las mismas del inventario, cruce por número+set) para que el panel reconozca cartas ya registradas por img; los demás usan TCGplayer (consistente con su inventario). Singles en Magic/YGO/PKM = tiene `Rarity` y NO `UPC` (su sellado sí trae extendedData). Precio base = 1º disponible de Normal/1st Edition/Holofoil/…; variante `foil` = Foil (MTG) / Reverse Holofoil (PKM). ⚠️ en `make_cartas.py` el `_rich` del catálogo solo RELLENA huecos (`setdefault`) — los CSVs de RB mandan (si pisara, cambiarían los slugs/URLs de las fichas).
...
- **Stock foil aparte:** campo `stockf` en `productos.json` = unidades de la variante foil. **⚠️ Si NO está definido, el foil está AGOTADO (0) — NUNCA hereda el stock normal.** (La regla vieja de heredar creaba "foils fantasma": 118 cartas con precio foil que Andrés no tenía físicamente se veían comprables; migrado 2026-07-12 con `stockf: 0` explícito, backup `productos_backup_foilstock.json`.) La tienda (card/quick-view/ficha/carrito), la API (`stockVariante`) y el panel respetan esa semántica; el panel edita el stock foil con el stepper ✨ FOIL (sin definir se muestra 0).
...
- **`EXPANSIONES.md`** — **playbook para agregar una expansión nueva** (TCGCSV → catálogo → productos.json a stock 0 → fichas → integridad → precios → push). Probado con Vendetta.
...
9. **REGLA — cartas nuevas = base + catálogo del panel + ficha + BUSCABLES.** Cada vez que se agreguen cartas (una expansión nueva, o lo que Andrés suba por el panel) tienen que quedar en los CUATRO lados: `productos.json` (base), `catalogo/<juego>.json` (panel, espejo por `img`), su ficha `/carta/`, y **encontrables** en la tienda. Flujo: `python make_catalogo.py <juego>` → agregar a `productos.json` a **stock 0** (Andrés sube el stock desde el panel según lo que tenga físico) → `python make_cartas.py`. **📘 Proceso completo paso a paso en [EXPANSIONES.md](EXPANSIONES.md)** (probado con Vendetta) — seguirlo cuando salga una expansión nueva.
   - **Visibilidad (decidida con Andrés 2026-07-24):** navegar el catálogo sin pedir nada muestra **solo lo comprable** (se conserva su regla de «stock 0 no aparece»); en cuanto el cliente **busca o filtra algo específico**, se ven TODAS con las agotadas marcadas «Agotado» → `userNarrowed()` en `js/app.js`. Los **conteos de los dropdowns siempre incluyen agotadas**, si no una expansión entera sin stock (ej. Vendetta) desaparecería del filtro. El buscador nunca las excluye (ordena disponibles primero + tag `.sr__out`).
